...
from pytorch_lightning.callbacks import ModelCheckpoint, EarlyStopping, TensorBoardLogger
...
class LightningTrainer(pl.LightningModule):
    ...
    def _step(
        ...
        # 记录损失和指标到 TensorBoard
        for key, value in losses.items():
            self.log(f"{prefix}/{key}", value, on_epoch=True, on_step=False, prog_bar=True)
...
#     dirpath="logs/checkpoints",  # 保存模型的文件夹路径
#     filename="best_model",  # 模型的文件名
#     save_top_k=1,  # 只保存最好的模型
#     mode="min",  # 选择较小的损失值作为最好的模型
# )

# early_stop_callback = EarlyStopping(
#     monitor="val/loss",  # 监控验证损失
#     patience=5,  # 如果在 5 个 epoch 内没有改善，就停止训练
#     mode="min",  # 选择较小的损失作为最佳模型
# )

# # 设置训练器
# trainer = pl.Trainer(
#     max_epochs=20,  # 设置最大训练轮数
#     logger=self.tb_logger,  # 使用 TensorBoardLogger
#     callbacks=[checkpoint_callback, early_stop_callback],  # 加入回调函数
# )
