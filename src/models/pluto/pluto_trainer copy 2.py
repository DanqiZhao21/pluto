...
class LightningTrainer(pl.LightningModule):
    ...
    def _step(
        ...
        for scenario in scenarios:
            scenario_data = {
                "initial_LidarPc_token": scenario.token, # 获取 该scenario 起始的lidarPc的信息
                "log_name": scenario.log_name,          # 获取 log 文件名称
...
######################################################################################################################
        ...
        res = self.forward(features["feature"].data)#轨迹预测的结果
        # print("Shape of res:", res.shape)  # 打印整个形状AttributeError: 'dict' object has no attribute 'shape'
        losses = self._compute_objectives(res, features["feature"].data,scenarios_info)#预测结果和真值之间计算Loss
        metrics = self._compute_metrics(res, features["feature"].data, prefix)
        self._log_step(losses["loss"], losses, metrics, prefix)
        return losses["loss"] if self.training else 0.0

    def _compute_objectives(self, res, data,scenario_info) -> Dict[str, torch.Tensor]:
        '''
...
#从data中提取目标数据
        ...
        ###############################################################
        # import ipdb; ipdb.set_trace()  # 进入调试模式
        # print("Batch size (bs):", data["agent"]["target"][:train_num].shape)  # 打印第一个维度，即 batch size
        # print("Batch size (bs):", data["agent"]["target"][:train_num].shape)
        # print("Batch size (bs):", data["agent"]["target"][:train_num].shape)
        ###########################################################################
        #语法解释：
    ...
    ####################################################################################################
        ...
        ego_reg_loss, ego_cls_loss, collision_loss = self.get_planning_loss(
            data, trajectory, probability, valid_mask[:, 0], target[:, 0], train_num, scenario_info
        )
    ...
    def get_planning_loss(self, data, trajectory, probability, valid_mask, target, bs,scenario_info):
